...
# Rampe de gravite : indigo clair -> indigo -> violet -> carmin
ECHELLE          = [[0.0, "#e4e7ff"], [0.30, "#a5b0f5"], [0.55, "#6366f1"],
                    [0.78, "#7c3aed"], [1.0, "#be123c"]]
VUE_GENERALE     = ""
SEP              = "\x1f"
...
#  SYSTEME DE DESIGN
# =====================================================================
PALETTE = ["#4f46e5", "#0d9488", "#059669", "#e11d48", "#7c3aed",
           "#b45309", "#0891b2", "#64748b", "#ca8a04", "#2563eb"]

_DOUX = ["#4f46e5", "#0d9488", "#b45309", "#7c3aed", "#0891b2"]

_ENCRE, _ACCENT, _OK = "#0f172a", "#e11d48", "#2563eb"
_BLEU, _GRILLE, _GRIS = "#6366f1", "#eef2f7", "#64748b"
_VIDE = "rgba(0,0,0,0)"
...
_IC_FILTRE = ("M3 5h18l-7 8.2V20l-4-2.2v-4.6L3 5z")
_IC_BARRES = ("M4 20h3V10H4v10zm6.5 0h3V4h-3v16zM17 20h3v-7h-3v7z")
_IC_COURBE = ("M3 3v18h18v-2H5V3H3zm4 12l3.5-4.5 3 3.5L19 7l1.5 1.2-5.9 "
              "7.6-3-3.5L8.4 17 7 15z")
_IC_TABLE  = ("M3 4h18v4H3V4zm0 6h18v4H3v-4zm0 6h18v4H3v-4z")
_IC_LOUPE  = ("M15.5 14h-.8l-.3-.3a6.5 6.5 0 10-.7.7l.3.3v.8l5 5 1.5-1.5-5-5zm"
              "-6 0a4.5 4.5 0 110-9 4.5 4.5 0 010 9z")
...
_POLICE = ("Inter, -apple-system, BlinkMacSystemFont, 'Segoe UI', "
           "Roboto, system-ui, sans-serif")


def _mise_en_forme(fig, hauteur, marges=None):
    fig.update_layout(
        template="plotly_white", paper_bgcolor=_VIDE, plot_bgcolor=_VIDE,
        font=dict(family=_POLICE, size=12.5, color=_TEXTE_DOUX),
        height=hauteur, autosize=True, separators=", ",
        hoverlabel=dict(bgcolor="white", bordercolor=_BORDURE, align="left",
                        font=dict(family=_POLICE, size=12.5, color=_ENCRE)),
        margin=marges or dict(l=10, r=40, t=95, b=45))
    fig.update_xaxes(zeroline=False, linecolor=_BORDURE,
                     tickfont=dict(size=11.5, color=_TEXTE_DOUX),
                     title_font=dict(size=11.5, color=_TEXTE_DOUX))
    fig.update_yaxes(zeroline=False, linecolor=_BORDURE,
                     tickfont=dict(size=11.5, color=_TEXTE_DOUX),
                     title_font=dict(size=11.5, color=_TEXTE_DOUX))
    return fig
...
def _bandeau(titre, sous_titre=None, coul=_ACCENT_BLEU, icone=_IC_FILTRE,
             ancre=None):
    return html.Div(
        **({"id": ancre} if ancre else {}), className="bandeau",
        style={"display": "flex", "alignItems": "center", "gap": "13px",
               "background": _FOND_CARTE, "borderRadius": _RAYON,
               "border": f"1px solid {_BORDURE}",
               "borderLeft": f"3px solid {coul}",
               "padding": "13px 18px", "margin": "26px 0 13px 0",
            ...
            html.Div([
                html.Div(titre,
                         style={"fontSize": "14.5px", "fontWeight": "700",
                                "color": _TITRE_PRINC, "letterSpacing": ".1px",
                                "lineHeight": "1.3"}),
                html.Div(sous_titre,
                         style={"fontSize": "12px", "color": _TEXTE_DOUX,
                                "marginTop": "2px", "fontWeight": "450"})
                if sous_titre else None,
            ]),
        ])


def _panneau(contenu, marge_bas="18px", pad="6px"):
    return html.Div(
        contenu, className="panneau",
        style={"background": _FOND_CARTE, "borderRadius": _RAYON,
               "border": f"1px solid {_BORDURE}", "boxShadow": _OMBRE_CARTE,
               "padding": pad, "marginBottom": marge_bas,
               "overflow": "hidden"})


def _libelle_controle(icone, titre, explication):
    """Icone + NOM : ce que fait la selection."""
    return html.Div(
        style={"display": "flex", "alignItems": "center", "gap": "7px",
               "marginBottom": "8px", "flexWrap": "wrap", "lineHeight": "1.4"},
        children=[
            html.Div(_icone(icone, _ACCENT_INDIGO, 13),
                     style={"width": "22px", "height": "22px",
                            "borderRadius": "7px", "background": "#f0f0ff",
                            "display": "flex", "alignItems": "center",
                            "justifyContent": "center", "flex": "0 0 auto"}),
            html.Span(titre, style={"fontSize": "10.5px", "fontWeight": "700",
                                    "color": _TITRE_PRINC,
                                    "textTransform": "uppercase",
                                    "letterSpacing": ".7px"}),
            html.Span(f": {explication}",
                      style={"fontSize": "12px", "color": _TEXTE_DOUX,
                             "fontWeight": "450"}),
        ])
...
def _lecture(*elements):
    """Ligne de legende placee au-dessus d'un graphique."""
    return html.Div(
        [html.Span("Lecture", style={"fontWeight": "700", "color": _TITRE_PRINC,
                                     "marginRight": "12px",
                                     "textTransform": "uppercase",
                                     "letterSpacing": ".6px",
                                     "fontSize": "10.5px"})] + list(elements),
        style={"display": "flex", "alignItems": "center", "flexWrap": "wrap",
               "rowGap": "6px", "fontSize": "12px", "color": _TEXTE_DOUX,
...
_SECTIONS = [("apercu", "Vue d'ensemble", _IC_STATS),
             ("perimetre", "Périmètre", _IC_FILTRE),
             ("atypiques", "Atypiques", _IC_BARRES),
             ("intervalles", "Intervalles", _IC_CIBLE),
             ("historique", "Historique", _IC_COURBE),
             ("maille-fine", "Maille fine", _IC_LOUPE),
             ("variables", "Variables", _IC_ALERTE),
             ("priorisation", "Priorisation", _IC_TABLE)]


def _navigation():
    return html.Div(className="nav-collante", children=[
        html.A([_icone(ic, _TEXTE_DOUX, 14), html.Span(lib)],
               href=f"#{ancre}", className="lien-nav")
        for ancre, lib, ic in _SECTIONS])


def _pied(socle):
    return html.Div(className="pied", children=[
        html.Div([
            html.Span("Sources", style={"fontWeight": "700",
                                        "color": _TITRE_PRINC,
                                        "marginRight": "8px"}),
            html.Span(f"valeurs observées : df_model ({socle.target})  ·  "
                      "score_composite, prédiction et bornes : anomalies_prio  ·  "
                      f"période validée : {socle.libelle_periode('carte')}  ·  "
                      f"couverture cible : {100 * (1 - socle.alpha):.0f} %"),
        ]),
        html.Div(f"Anomaly Detection Dashboard  ·  Conformal Prediction  ·  "
                 f"généré le {datetime.now():%d/%m/%Y} à "
                 f"{datetime.now():%H:%M}",
                 style={"marginTop": "4px", "opacity": ".75"}),
    ], style={"marginTop": "34px", "padding": "18px 4px 4px 4px",
              "borderTop": f"1px solid {_BORDURE}", "fontSize": "11.5px",
              "color": _TEXTE_DOUX, "lineHeight": "1.6"})


# =====================================================================
#  Gabarit HTML + feuille de style globale
# =====================================================================
  ...
  <head>
    ...
    <style>
      * { box-sizing: border-box; }
      ...
      /* ---- navigation collante ---- */
      .nav-collante { position:sticky; top:10px; z-index:40;
        display:flex; gap:4px; overflow-x:auto; scrollbar-width:none;
        padding:6px; margin:0 0 6px 0; border-radius:14px;
        background:rgba(255,255,255,.80);
        -webkit-backdrop-filter:saturate(180%) blur(14px);
        backdrop-filter:saturate(180%) blur(14px);
        border:1px solid #e6eaf2;
        box-shadow:0 1px 2px rgba(15,23,42,.04),
                   0 10px 28px -16px rgba(15,23,42,.22); }
      .nav-collante::-webkit-scrollbar { display:none; }
      .lien-nav { display:inline-flex; align-items:center; gap:7px;
        padding:8px 13px; border-radius:10px; white-space:nowrap;
        font-size:12.5px; font-weight:600; color:#475569;
        text-decoration:none; transition:background .15s, color .15s; }
      .lien-nav:hover { background:#f0f0ff; color:#3730a3; }
      .lien-nav img { opacity:.8; }
      .bandeau { scroll-margin-top:78px; }

      /* ---- tracker ---- */
      .barre-tracker { transition:transform .12s ease, filter .12s ease;
        cursor:default; }
      .barre-tracker:hover { transform:scaleY(1.18); filter:brightness(.92); }

      /* ---- apparition douce ---- */
      .panneau, .carte-kpi, .bandeau { animation:apparition .45s ease backwards; }
      ...
      body { font-family:Inter,-apple-system,BlinkMacSystemFont,'Segoe UI',
        Roboto,system-ui,sans-serif; color:#334155; }
      ...
      /* ---- panneaux et cartes ---- */
      .panneau { transition:box-shadow .22s ease, transform .22s ease; }
      .carte-kpi { transition:box-shadow .22s ease, transform .22s ease; }
      .carte-kpi:hover { transform:translateY(-2px);
        box-shadow:0 2px 4px rgba(15,23,42,.05),
                   0 18px 34px -18px rgba(15,23,42,.30); }

      /* ---- puces des axes d'agregation ---- */
      .puces-axes label:hover { border-color:#c7d2fe !important;
        background:#f8faff !important; }
      .puces-axes label:has(input:checked) {
        border-color:#4f46e5 !important; background:#f0f0ff !important;
      ...
      /* ---- listes deroulantes : Dash 2/3 (react-select v1) ---- */
      .Select-control { border-radius:10px !important;
        border:1px solid #e6eaf2 !important; min-height:42px !important;
        box-shadow:none !important; background:#fff !important;
        transition:border-color .15s ease, box-shadow .15s ease; }
      .Select-control:hover { border-color:#c7d2fe !important; }
      .is-focused:not(.is-open) > .Select-control {
        border-color:#4f46e5 !important;
        box-shadow:0 0 0 3px rgba(79,70,229,.12) !important; }
      .Select-placeholder, .Select--single > .Select-control .Select-value {
        line-height:40px !important; padding-left:14px !important; }
      .Select-placeholder { color:#94a3b8 !important; font-size:13px; }
      .Select-value-label { color:#0f172a !important; font-size:13px;
        font-weight:550; }
      .Select-input > input { padding:11px 0 !important; font-size:13px; }
      .Select-menu-outer { z-index:60 !important;
        border-radius:12px !important; margin-top:5px;
        border:1px solid #e6eaf2 !important; overflow:hidden;
        box-shadow:0 16px 38px -12px rgba(15,23,42,.24) !important; }
      .Select-option { font-size:12.8px !important; padding:10px 14px !important;
        color:#334155 !important; }
      .Select-option.is-focused { background:#f0f0ff !important; }
      .Select-option.is-selected { background:#e6e6ff !important;
        color:#3730a3 !important; font-weight:600; }
      .Select--multi .Select-value { background:#f0f0ff !important;
        border:1px solid #c7d2fe !important; border-radius:7px !important;
        color:#3730a3 !important; font-size:12px !important;
        margin:5px 4px 0 5px !important; }
      .Select--multi .Select-value-icon {
        border-right:1px solid #c7d2fe !important; padding:2px 6px !important; }
      .Select--multi .Select-value-icon:hover { background:#dcd9ff !important;
        color:#312e81 !important; }
      .Select-arrow { border-color:#94a3b8 transparent transparent !important; }

      /* ---- listes deroulantes : Dash 4 ---- */
      .dash-dropdown .dash-dropdown-trigger {
        border-radius:10px !important; border:1px solid #e6eaf2 !important;
        min-height:42px !important; background:#fff !important;
        padding:0 12px !important; font-size:13px !important;
        box-shadow:none !important;
        transition:border-color .15s ease, box-shadow .15s ease; }
      .dash-dropdown .dash-dropdown-trigger:hover {
        border-color:#c7d2fe !important; }
      .dash-dropdown .dash-dropdown-trigger:focus-within {
        border-color:#4f46e5 !important;
        box-shadow:0 0 0 3px rgba(79,70,229,.12) !important; }
      .dash-dropdown-value-item { color:#0f172a !important; font-weight:550; }
      .dash-dropdown-menu, .dash-dropdown-options {
        border-radius:12px !important; border:1px solid #e6eaf2 !important;
        box-shadow:0 16px 38px -12px rgba(15,23,42,.24) !important;
        overflow:hidden; margin-top:5px; }
      .dash-dropdown-option { font-size:12.8px !important;
        padding:10px 14px !important; }
      .dash-dropdown-option:hover, .dash-dropdown-option[data-highlighted] {
        background:#f0f0ff !important; }

      /* ---- tableau ---- */
      .dash-table-container .dash-spreadsheet-container
        .dash-spreadsheet-inner table { border-collapse:separate !important; }
      .dash-table-container .dash-spreadsheet-container
        .dash-spreadsheet-inner tr:hover td { filter:brightness(.985); }

      /* ---- graphiques ---- */
      .js-plotly-plot .plotly .modebar { opacity:0; transition:opacity .2s; }
      .panneau:hover .js-plotly-plot .plotly .modebar { opacity:.7; }

      @media (max-width: 720px) {
        .grille-kpi { grid-template-columns:repeat(auto-fit,minmax(150px,1fr)) !important; }
      }
    </style>
  </head>
  <body>
...
class _Socle:
    def __init__(self, anomalies_prio, expl, df, id_cols, target, alpha):
        ...
        # recente de df_model. Source unique pour cartes, barres et courbes.
        ...
        # --- y_obs de chaque ligne d'expl : valeur de df_model a la periode ---
        if "y_obs" in self.ex.columns and target in df.columns:
            obs = self._obs_df(self.cles).rename(columns={"y_obs": "_obs_df"})
            res = self.ex[self.cles].merge(obs, on=self.cles, how="left")
            trouve = res["_obs_df"].notna().to_numpy()
            self.ex["y_obs"] = np.where(trouve, res["_obs_df"].to_numpy(), np.nan)
            self.diagnostic.append(
                f"y_obs : {int(trouve.sum())}/{len(self.ex)} lignes d'expl "
                f"retrouvees dans df_model a la periode {self.libelle_periode()}")
            if not trouve.all():
                exemple = " | ".join(self.ex.loc[~trouve, self.cles].iloc[0])
                self.diagnostic.append(
                    f"  sans correspondance (y_obs laisse vide), ex. : {exemple}")
    ...
    def table_axes(self, sub, axes, filtre=None):
        """Une ligne par groupe d'axes contenant au moins une anomalie du
        perimetre. Les montants portent sur TOUS les sous-portefeuilles du
        groupe dans le perimetre, comme la courbe d'evolution :
          - y_obs : df_model a la periode validee (meme calcul que la courbe) ;
          - y_pred, bornes : somme d'expl sur ces memes sous-portefeuilles ;
          - score_composite : repris d'anomalies_prio (max du groupe)."""
...
def _creer_barres():
    fig = go.Figure(go.Bar(x=[], y=[], orientation="h", showlegend=False,
                           ...
                           textposition="none", width=.68))
    fig.update_layout(xaxis=dict(tickformat="~s", gridcolor=_GRILLE),
                      yaxis=dict(tickfont=dict(size=11)), bargap=.3)
    _mise_en_forme(fig, 520, dict(l=10, r=30, t=64, b=50))
    return fig

def _creer_forest(target):
    fig = go.Figure()
    fig.add_scatter(x=[], y=[], mode="lines", hoverinfo="skip", opacity=.55,
                    line=dict(color="#c7d2fe", width=9),
                    name="Intervalle conforme")
    fig.add_scatter(x=[], y=[], mode="lines", showlegend=False, hoverinfo="skip",
                    line=dict(color=_ACCENT, width=1.6, dash="dot"))
    fig.add_scatter(x=[], y=[], mode="markers", name="Prédiction",
                    marker=dict(symbol="diamond", size=9, color="white",
                                line=dict(color=_TEXTE_DOUX, width=1.5)))
    fig.add_scatter(x=[], y=[], mode="markers", name="Valeur observée",
                    marker=dict(size=12, color=_ACCENT,
                                line=dict(color="white", width=1.6)))
    fig.update_xaxes(tickformat="~s", title_text=target, gridcolor=_GRILLE)
    fig.update_layout(showlegend=False)     # legende portee par la ligne « Lecture »
    _mise_en_forme(fig, 520, dict(l=10, r=40, t=50, b=55))
    return fig
...
def _fig_cercle(socle, sub, titre, axes):
    ...
    if not len(h):
        return _vider(fig, "Aucune anomalie à représenter.", 300)
    ...
    t.marker = dict(
        ...
        cmin=0, cmax=cmax, line=dict(color="white", width=1.8),
        colorbar=dict(title=dict(text="score_composite<br>moyen",
                                 font=dict(size=11, color=_TEXTE_DOUX),
                                 side="right"),
                      thickness=12, len=.62, outlinewidth=0,
                      tickfont=dict(size=10.5, color=_TEXTE_DOUX),
                      tickformat="~s", x=1.0))
    t.insidetextfont = dict(family=_POLICE, size=11, color="white")
    fig.layout.height = 620
    fig.layout.title = dict(
        text=f"<span style='font-size:12.5px;color:{_TEXTE_DOUX}'>"
             f"{' › '.join(axes)}  ·  {titre}</span>",
...
def _cartes(socle, sub, sub_ex, titre, filtre=None):
    ...
    cible = 1 - socle.alpha
    obs_ok = ex["y_obs"].notna() if "y_obs" in ex.columns else pd.Series(dtype=bool)
    if n_perim and obs_ok.any():
        e = ex[obs_ok]
        couverture = float(((e["y_obs"] >= e["borne_basse"])
                            & (e["y_obs"] <= e["borne_haute"])).mean())
    else:
...
def _fig_barres(table, titre, axes, target, top_n):
    ...
    survol = [
        f"<b>{r['libelle']}</b><br>{target} observé : {_fmt(r['y_obs'])}"
        f"<br>Prédit : {_fmt(r['y_pred'])}"
        f"<br>Intervalle : [{_fmt(r['lo'])} ; {_fmt(r['hi'])}]"
        f"<br>score_composite : {_fmt4(r[COL_SCORE])}"
        f"<br>{int(r['n'])} anomalie(s) sur {int(r['n_lignes'])} "
        f"sous-portefeuille(s) du groupe"
        for _, r in g.iterrows()]
    montants = g["y_obs"].tolist()
    couvert = g["couvert"].tolist() if "couvert" in g.columns else [True] * len(g)
    t = fig.data[0]
    t.x, t.y = montants, g["libelle"].tolist()
    # La longueur porte deja le montant : la couleur sert le statut de couverture
    t.marker.color = ["rgba(37,99,235,.82)" if c else "rgba(225,29,72,.88)"
                      for c in couvert]
    t.text, t.hovertemplate = survol, "%{text}<extra></extra>"
    fig.layout.xaxis.title.text = target
    fig.layout.height = max(380, 38 * len(g) + 120)
    n_hors = int(sum(1 for c in couvert if not c))
    fig.layout.title = dict(
        text=f"<span style='font-size:12.5px;color:{_TEXTE_DOUX}'>{titre}"
             f"  ·  {len(g)} sous-portefeuilles  ·  "
             f"<span style='color:{_ACCENT}'>{n_hors} hors intervalle</span>"
             f"</span>",
        x=.006, xanchor="left", y=.985, yanchor="top")
    return fig
...
def _fig_forest(table, titre, axes, target, top_n):
    ...
    textes = [
        f"<b>{lab}</b><br>{target} observé : {_fmt(o)}<br>Prédit : {_fmt(p)}"
    ...
    dedans = (obs >= lo) & (obs <= hi)
    fig.data[3].marker.color = [_OK if c else _ACCENT for c in dedans]
    fig.layout.xaxis.title.text = target
    fig.layout.yaxis = dict(tickmode="array", tickvals=y,
                            ticktext=ticks, tickfont=dict(size=11),
                            showgrid=False)
    fig.layout.height = max(420, 44 * len(d) + 150)
    fig.layout.title = dict(
        text=f"<span style='font-size:12.5px;color:{_TEXTE_DOUX}'>{titre}"
             f"  ·  {int((~dedans).sum())} observations hors intervalle sur "
             f"{len(d)}</span>",
        x=.006, xanchor="left", y=.985, yanchor="top")
    return fig
...
def _remplir_evolution(fig, hist, per, ctx_, valeurs, axes, target, alpha,
    ...
    fig.data[5].name = (("Couvert" if couvert else "Hors intervalle")
                        if conforme else "Valeur observée")
    ...
    fig.layout.title = dict(
        text=f"<b style='color:{_ENCRE}'>{' | '.join(valeurs)}</b>"
             ...
             f"axes {' + '.join(axes)}"
             + (f" · période validée <b>{p_valide}</b>" if (conforme and ctx_)
                else "")
             + "</span>",
...
def _fig_evolution_fine(socle, sub, target, alpha, valeurs_fines=None):
    """Graphe 2 : sous-portefeuille a la maille la plus fine (cle complete),
    avec prediction et intervalle conforme. Ne depend que du perimetre
    (maille/valeur) et de ses propres controles, jamais des axes d'agregation.
    Sans selection explicite, retombe sur la pire anomalie du perimetre."""
    fig = _creer_evolution(target)
    if sub is None or not len(sub):
        return _vider(fig, "Aucune anomalie à la maille la plus fine.")
    if not valeurs_fines:
        pire = sub.loc[sub[COL_SCORE].idxmax()]
        valeurs_fines = [str(pire[c]) for c in socle.cles]
    valeurs_fines = list(valeurs_fines)
    # a la cle complete, le perimetre est deja porte par la cle elle-meme
    hist, per, _ = socle.historique(valeurs_fines, socle.cles)
    table_fine = socle.table_axes(sub, socle.cles)
...
def _tableau(table, titre):
    if not len(table):
        return html.P(f"Aucune anomalie dans le périmètre : {titre}",
                      style={"padding": "18px 20px", "color": _TEXTE_DOUX})
    try:
        ...
        t = pd.DataFrame({
            ...
            "Observé": [_fmt(v) for v in d["y_obs"]],
            "Prédiction": [_fmt(v) for v in d["y_pred"]],
            "Borne inf.": [_fmt(v) for v in d["lo"]],
            "Borne sup.": [_fmt(v) for v in d["hi"]],
            "Statut": ["Couvert" if c else "Hors intervalle"
                       for c in d["couvert"]],
            "score_composite": [_fmt4(v) for v in d[COL_SCORE]],
        })
        scores = d[COL_SCORE].to_numpy(dtype="float64")
        smin, smax = float(scores.min()), float(scores.max())
        etendue = (smax - smin) or 1.0
        degrade = [{"if": {"row_index": "odd"},
                    "backgroundColor": "#fbfcfe"}]
        degrade += [{"if": {"filter_query": '{Statut} = "Hors intervalle"',
                            "column_id": "Statut"},
                     "color": _ACCENT, "fontWeight": "650"},
                    {"if": {"filter_query": '{Statut} = "Couvert"',
                            "column_id": "Statut"},
                     "color": _ACCENT_VERT, "fontWeight": "650"}]
        degrade += [{"if": {"row_index": i, "column_id": "score_composite"},
                     "backgroundColor":
                         f"rgba(225,29,72,{0.07 + 0.34 * (scores[i]-smin)/etendue:.3f})",
                     "color": "#9f1239", "fontWeight": "700"}
                    for i in range(n)]
        return html.Div([
            dash_table.DataTable(
                ...
                style_cell={"fontFamily": _POLICE, "fontSize": "12.5px",
                            "padding": "10px 13px", "border": "none",
                            "color": _TEXTE, "textAlign": "right",
                            "borderBottom": f"1px solid {_GRILLE}",
                            "fontVariantNumeric": "tabular-nums"},
                style_cell_conditional=[
                    {"if": {"column_id": "Maille"}, "textAlign": "left",
                     "fontWeight": "600", "color": _TITRE_PRINC,
                     "minWidth": "180px"},
                    {"if": {"column_id": "Rang"}, "textAlign": "center",
                     "width": "54px", "fontWeight": "700",
                     "color": _TEXTE_DOUX},
                    {"if": {"column_id": "Statut"}, "textAlign": "center",
                     "width": "120px"},
                    {"if": {"column_id": "score_composite"},
                     "textAlign": "center", "width": "120px"},
                ],
                style_header={"backgroundColor": "#fbfcfe",
                              "fontWeight": "650", "color": _TEXTE_DOUX,
                              "fontSize": "10.5px", "border": "none",
                              "textTransform": "uppercase",
                              "letterSpacing": ".6px", "padding": "12px 16px",
                              "borderBottom": f"1px solid {_BORDURE}"},
                style_data_conditional=degrade,
            ),
        ])
    except Exception as e:
        return html.P(f"Tableau non généré : {type(e).__name__} : {str(e)[:150]}")
                ...
                children=[
                    html.Div([
                        ...
                        html.Div([
                            ...
                            _pastille_entete(_IC_LIGNES, "Observé : df_model"),
...
def configurer_app(app, anomalies_prio, expl, df, id_cols, target,
    ...
    axes_checklist = dcc.Checklist(
        ...
        inline=True,
        className="puces-axes",
        inputStyle={"marginRight": "7px", "accentColor": _ACCENT_INDIGO,
                    "cursor": "pointer"},
        labelStyle={"display": "inline-flex", "alignItems": "center",
                    "padding": "8px 15px", "margin": "0 8px 8px 0",
                    "border": f"1px solid {_BORDURE}",
                    "borderRadius": "10px", "background": "#fff",
                    "cursor": "pointer", "fontSize": "12.5px",
                    "fontWeight": "600", "color": _TEXTE,
    ...
    # Controles propres au graphe fin : ils ne pilotent que ce graphe
    sel_unite_fine = dcc.Dropdown(
        id="sel-unite-fine", options=[], value=None, clearable=False,
        placeholder="Sous-portefeuille à la maille la plus fine…")
    ...
    sel_var_extra = dcc.Dropdown(
        id="sel-var-extra", options=[],
        value=None, clearable=True, multi=True,
        placeholder="Ajouter une ou plusieurs variables explicatives…")

    def _graphe(idf, figure):
        return dcc.Graph(id=idf, figure=figure,
    ...
    fig_cercle = _graphe("fig-cercle", _creer_cercle())
    fig_barres = _graphe("fig-barres", _creer_barres())
    fig_forest = _graphe("fig-forest", _creer_forest(target))
    fig_evol   = _graphe("fig-evol", _creer_evolution(target))
    fig_evol_fine = _graphe("fig-evol-fine", _creer_evolution(target))
    fig_vars   = _graphe("fig-vars", _creer_variables())
    ...
    # -- Layout --
    _BLEU_OK, _ROUGE_KO = "rgba(37,99,235,.82)", "rgba(225,29,72,.88)"

    app.layout = html.Div(
        className="page",
        style={"fontFamily": _POLICE, "background": _FOND_PAGE,
               "minHeight": "100vh", "padding": "26px 0 40px 0",
               "color": _TEXTE},
        children=[html.Div(
            ...
            _bandeau("Vue d'ensemble",
                     "Indicateurs clés du périmètre sélectionné : ils se "
                     "recalculent à chaque changement de filtre",
                     _ACCENT_BLEU, _IC_STATS, ancre="apercu"),
            cartes_div,

            _bandeau("Périmètre d'analyse",
                     "Réglez ces trois filtres : tous les graphiques et le "
                     "tableau de la page se mettent à jour",
                     _ACCENT_VERT, _IC_FILTRE, ancre="perimetre"),
            _panneau(html.Div([
                html.Div([_libelle_controle(
                    _IC_BARRES, "Axes d'agrégation",
                    "cochez les dimensions qui découpent le cercle, les "
                    "barres, les intervalles et le tableau"), axes_checklist],
                    style={"marginBottom": "16px"}),
                html.Div([
                    html.Div([_libelle_controle(
                        _IC_FILTRE, "Maille",
                        "dimension sur laquelle restreindre le périmètre"),
                        sel_maille],
                        style={"flex": "1 1 300px", "minWidth": "240px"}),
                    html.Div([_libelle_controle(
                        _IC_CIBLE, "Valeur",
                        "modalité retenue ; « General view » garde tout le "
                        "portefeuille"), sel_valeur],
                        style={"flex": "2 1 420px", "minWidth": "280px"}),
                ], style={"display": "flex", "gap": "18px",
                          "flexWrap": "wrap"}),
            ]), pad="18px 20px 20px 20px"),
            _panneau([
                _lecture(html.Span("taille d'un secteur = poids de ses "
                                   "anomalies ; couleur = score_composite "
                                   "moyen ; cliquez un secteur pour filtrer "
                                   "la page")),
                fig_cercle]),

            _bandeau("Identification des observations atypiques",
                     "Montant observé (df_model) des groupes qui contiennent "
                     "au moins une anomalie, classés par score_composite",
                     _ACCENT_ORANGE, _IC_BARRES, ancre="atypiques"),
            _panneau([
                _lecture(_puce(_BLEU_OK, "observé dans l'intervalle", "carre"),
                         _puce(_ROUGE_KO, "observé hors intervalle", "carre"),
                         html.Span("longueur = montant observé à la période "
                                   "validée, identique au dernier point de "
                                   "l'historique")),
                fig_barres]),

            _bandeau("Conformal Prediction — intervalles",
                     "Chaque groupe comparé à son intervalle conforme : un "
                     "point rouge sort de la plage attendue par le modèle",
                     _ACCENT_VIOLET, _IC_CIBLE, ancre="intervalles"),
            _panneau([
                _lecture(_puce("#c7d2fe", "intervalle conforme", "carre"),
                         _puce(_TEXTE_DOUX, "prédiction"),
                         _puce(_OK, "observé couvert"),
                         _puce(_ACCENT, "observé hors intervalle")),
                fig_forest]),

            _bandeau("Historique de l'observation",
                     "Évolution observée du groupe choisi, dans le périmètre "
                     "courant ; son dernier point est la valeur des barres",
                     _ACCENT_TEAL, _IC_COURBE, ancre="historique"),
            _panneau(html.Div([_libelle_controle(
                _IC_COURBE, "Sous-portefeuille",
                "groupe dont vous suivez l'évolution ; la liste suit les "
                "axes d'agrégation"), sel_unite]),
                     marge_bas="14px", pad="16px 20px 18px 20px"),
            _panneau(fig_evol),

            _bandeau("Maille la plus fine — prédiction et intervalle conforme",
                     "Un sous-portefeuille précis, sans agrégation ; ces "
                     "réglages ne modifient aucun autre graphique",
                     _ACCENT_INDIGO, _IC_LOUPE, ancre="maille-fine"),
            _panneau(html.Div([
                html.Div([_libelle_controle(
                    _IC_LOUPE, "Sous-portefeuille",
                    "une ligne à la clé complète ; par défaut, la plus grave "
                    "du périmètre"), sel_unite_fine],
                    style={"flex": "2 1 440px", "minWidth": "280px"}),
                html.Div([_libelle_controle(
                    _IC_RANG, "Classement",
                    "ordre de la liste ci-contre"), sel_tri_fine],
                    style={"flex": "1 1 260px", "minWidth": "220px"}),
            ], style={"display": "flex", "gap": "18px", "flexWrap": "wrap"}),
                     marge_bas="14px", pad="16px 20px 18px 20px"),
            _panneau(fig_evol_fine),

            _bandeau("Variables explicatives",
                     "Variables dont le dernier trimestre s'écarte le plus de "
                     "leur régime habituel",
                     _ACCENT_ROSE, _IC_ALERTE, ancre="variables"),
            _panneau(fig_vars, marge_bas="14px"),
            _panneau(html.Div([_libelle_controle(
                _IC_STATS, "Explorer d'autres variables",
                "ajoutez une ou plusieurs variables pour tracer leur "
                "évolution sous les cinq premières"), sel_var_extra]),
                     marge_bas="14px", pad="16px 20px 18px 20px"),
            html.Div(id="fig-vars-extra-div"),

            _bandeau("Tableau de priorisation",
                     "Groupes classés par score_composite (anomalies_prio) ; "
                     "statut calculé sur l'observé de df_model",
                     _ACCENT_BLEU, _IC_TABLE, ancre="priorisation"),
            _panneau(tableau_div, pad="0"),
    ...
    def maj_panneaux(axes_value, valeur, maille, unite_ancienne):
        ...
        table = socle.table_axes(sub, axes, filtre)
        etat["table"] = table
    ...
    # --- Controles propres au graphe a la maille la plus fine ---
    @app.callback(
    ...
    def maj_options_fine(valeur, tri, maille, ancienne):
        sub, _, _ = socle.filtrer(maille, valeur)
        if not len(sub):
            return [], None
        t = socle.table_axes(sub, socle.cles)
        if not len(t):
            return [], None
        if tri == "montant":
        ...
        for _, r in t.iterrows():
            cle = tuple(str(r[c]) for c in socle.cles)
            options.append({"label": f"{' | '.join(cle)}   ({_fmt(r['y_obs'])})",
                            "value": _encode_cle(socle.cles, cle)})
        dispo = [o["value"] for o in options]
    ...
    def maj_evol_fine(cle_encodee, maille, valeur):
        try:
            sub, _, _ = socle.filtrer(maille, valeur)
            valeurs_fines = None
            if cle_encodee:
                _, valeurs_fines = _decode_cle(cle_encodee)
            return _fig_evolution_fine(socle, sub, target, alpha, valeurs_fines)
        except Exception as e:
    ...
    def maj_unite(cle_encodee, maille, valeur):
        ...
        if cle_encodee is None:
            return (_vider(_creer_evolution(target),
                           "Aucun sous-portefeuille dans ce périmètre."),
        ...
        try:
            ...
            msg = f"Mise à jour impossible : {type(e).__name__} : {str(e)[:110]}"
    ...
    def maj_var_extra(noms_vars, cle_encodee):
        ...
        if hist is None or per is None:
            return ""
        return [_panneau(_fig_variable_unique(socle, hist, per, ctx_, v),
                         marge_bas="12px")
                for v in noms_vars]
...
if _manquants:
    ...
    for n in _manquants:
        print(f"  - {n}")
    app.layout = html.Div("Données manquantes : voir la console du run.")
else:
    try:
...
Code 222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222
